...
const workflow = {
     ...
     * 获取某业务记录对应的审批日志信息(历史)
     */
    async queryPRLogHistoryByDataID(business_data_id) {

        //提交URL
        var queryURL = `${window.BECONFIG['restAPI']}/apis/pr_log_history?_where=(business_data_id,eq,${business_data_id})&_sort=operate_time&_p=0&_size=1000`;
     ...
     * 获取某业务记录对应的审批日志信息(当前)
     */
    async queryPRLogByDataID(business_data_id) {
        //提交URL
        var queryURL = `${window.BECONFIG['restAPI']}/apis/pr_log?_where=(business_data_id,eq,${business_data_id})&_sort=operate_time&_p=0&_size=1000`;
     ...
     * 获取某业务记录对应的审批日志信息(知会)
     */
    async queryPRLogInformedByDataID(business_data_id) {
        //提交URL
        var queryURL = `${window.BECONFIG['restAPI']}/apis/pr_log_informed?_where=(business_data_id,eq,${business_data_id})&_sort=operate_time&_p=0&_size=1000`;
     ...
     * 获取某业务记录对应的审批日志信息(知会)
     */
    async queryPRLogUserByPage(page, size) {
        //提交URL
        var queryURL = `${window.BECONFIG['restAPI']}/apis/v_uname?_p=${page}&_size=${size}&_sort=username`;
     ...
     */
    async postWorkflowApprove(tableName, curRow, operationData, pnode, prLogHisNode, bpmStatus, freeNode, wflowAddUser, wflowNotifyUser, curAuditor) {
     ...
     */
    async approveViewProcessLog(prLogHisNode) {
     ...
     * @description 记录 审批人 经办人 审批表单 表单编号 记录编号 操作(同意/驳回) 意见 内容 表单数据
     */
    async taskViewProcessLog(prLogNode) {
        //记录 审批人 经办人 审批表单 表单编号 记录编号 操作(同意/驳回) 意见 内容 表单数据
     ...
     * @description 记录 审批人 经办人 审批表单 表单编号 记录编号 操作(同意/驳回) 意见 内容 表单数据
     */
    async deleteViewProcessLog(node) {
        //替换ID编号
        node.map(item => {
            item.oid = item.id;
            item.id = item.pid;
     ...
     * @param $confirm 提示函数
     */
    checkSubmitInfo(wfUsers, nfUsers, approver, pageType = 'workflowing') {
     ...
     * @define bpmStatus （1：待提交	2：审核中	3：审批中	4：已完成	5：已完成 10：已作废）
     */
    async postWorkflowFree(userInfo, tableName, curRow, freeWFNode, startFreeNode, nextWflowNode, bpmStatus) {
