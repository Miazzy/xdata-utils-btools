...
function cloneArr (arr) {
  return BesUtils.map(arr, deepClone)
}
...
function isLeapYear (date) {
  var year
  var currentDate = date ? BesUtils.toStringDate(date) : new Date()
...
function isMatch (obj, source) {
  ...
  if (sourceKeys.length) {
    if (BesUtils.includeArrays(objKeys, sourceKeys)) {
      return BesUtils.some(sourceKeys, function (key2) {
    ...
    if (isRegExp(val1)) {
      ...
      if (isObj1Arr || isObj2Arr ? isObj1Arr && isObj2Arr : val1.constructor === val2.constructor) {
        ...
        if (val1Keys.length === val2Keys.length) {
          return isUndefined(result) ? BesUtils.every(val1Keys, function (key, index) {
