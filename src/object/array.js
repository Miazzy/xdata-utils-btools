...
var setupDefaults = require('../main/setup')
...
function arrayShuffle (array) {
  ...
  for (; len >= 0; len--) {
    index = len > 0 ? BesUtils.random(0, len) : 0
...
function arraySum (array, iterate, context) {
  var result = 0
  var toNumber = BesUtils.toNumber
  context = context || this
...
function arrayMean (array, iterate, context) {
  return BesUtils.toNumber(arraySum(array, iterate, context || this) / baseExports.getSize(array))
}
...
function unzip (arrays) {
  var result = []
  var index = 0
  var len = BesUtils.max(arrays, function (item) {
